...
int main() {
    ...
    config->num_nodes = config->num_training;
    config->load_file = config->training_file;

    // Load nodes
    float** training = new float*[config->num_training];
    load_nodes(config, training);
    float** generated = nullptr; 
    generated = new float*[config->num_training_generated];

    // Create HNSW graph using training set
    HNSW* hnsw = NULL;
    hnsw = init_hnsw(config, training);
    if (config->load_graph_file) {
        load_hnsw_files(config, hnsw, training, true);
    } else {
        for (int i = 1; i < config->num_training; ++i) {
    ...
    for (int i = 0; i< config->num_training_generated; i++){
        ...
        for(int j = 0; j < config->dimensions; j++){
            generated[i][j] = u * training[index_first][j] + v * training[index_second][j] + w * training[index_third][j];
        }
    }
