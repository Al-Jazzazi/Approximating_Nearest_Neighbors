...
public:
    ...
    bool should_terminate(Config* config, std::priority_queue<std::pair<float, int>>& top_k, std::pair<float, int>& top_1, float close_squared, float far_squared, float far_extension_sqaured, float alpha_break, bool is_querying, int layer_num);
